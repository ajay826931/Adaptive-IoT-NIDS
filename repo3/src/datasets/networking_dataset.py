...
class NetworkingDataset(Dataset):
    """
    My custom Dataset class for PyTorch. 
    It pre-loads networking biflow data into memory for faster training.
    """
...
def get_data(full_path, num_tasks, nc_first_task, nc_incr_tasks, validation, shuffle_classes, 
    ...
    """
    My core data preparation function: handles splitting, parsing, and class ordering.
    """
    ...
    classes_txt_path = os.path.join(path, f"classes_{dataset_filename.split('.')[0]}.txt")
    if os.path.exists(classes_txt_path):
        labels = np.loadtxt(classes_txt_path, dtype=str)
    else:
