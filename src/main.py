...
def main(argv=None):
    tstart = time.time()
    # Core Arguments for my custom training pipeline
    parser = argparse.ArgumentParser(description='My Custom Incremental Learning NIDS Pipeline')

    # General execution configs
    ...
    ####################################################################################################################
    # MAIN TRAINING PIPELINE STARTS HERE
    ####################################################################################################################
    ...
    # Initialize data loaders for my datasets
    utils.seed_everything(seed=args.seed)
    ...
    for t, (_, ncla) in enumerate(taskcla):
        # If I want to stop early, I can use stop_at_task
        ...
        # Setup the model and optimizer for the current task
